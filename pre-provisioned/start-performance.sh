...
while getopts "j:u:p:n:p:i:b:k:t:d:r:h" opts; do
    case $opts in
    j)
        jmeter_setup=${OPTARG}
        ;;
...
run_performance_tests_options+=(" -c $concurrent_users -l $cloud_host_name -n $rds_host -r $db_username -s $db_password -v $mode")
